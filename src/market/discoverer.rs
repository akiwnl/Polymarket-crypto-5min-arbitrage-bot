...
/// Seconds in a 5-minute window (used by main and other modules to calculate window_end)
...
impl MarketDiscoverer {
    ...
    /// Calculate the start timestamp of the current 5-minute window (UTC)
    /// Windows align to 5-minute marks: 0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55 min
    pub fn calculate_current_window_timestamp(now: DateTime<Utc>) -> i64 {
        let ts = now.timestamp();
        (ts / FIVE_MIN_SECS) * FIVE_MIN_SECS
    }

    /// Calculate the start timestamp of the next 5-minute window (UTC)
    ...
    /// Get 5-minute markets for a given timestamp
    pub async fn get_markets_for_timestamp(&self, timestamp: i64) -> Result<Vec<MarketInfo>> {
        ...
        match self.gamma_client.markets(&request).await {
            Ok(markets) => {
                ...
                info!(count = valid_markets.len(), "found matching markets");
                Ok(valid_markets)
            }
            Err(e) => {
                warn!(error = %e, timestamp = timestamp, "market query failed, markets may not be created yet");
                Ok(Vec::new())
            }
        }
    }

    /// Parse market info, extract YES and NO token_id
    fn parse_market(&self, market: polymarket_client_sdk::gamma::types::response::Market) -> Option<MarketInfo> {
        // Check if market is active, orderbook enabled, and accepting orders
